...
{ //Extends Editor required
    ...
    private void OnSceneGUI() //Monobehaviour editor function
    {
        ...
        for (int i = 0; i < mc.waypoints.Length; i++)
        {
            if (!mc.waypoints[i])
                continue;
            guiPoint = HandleUtility.WorldToGUIPoint(mc.waypoints[i].position);                     //Convert a space in the world to the 2D GUI space
            rect = new Rect(guiPoint.x - 50f, guiPoint.y - 40, 100, 20);     //Create a scalable rect (In this case, hardcoded size)                                                             
            if (GUI.Button(rect, "Waypoint: " + i))   //Create a GUI button with the rect size
                Debug.Log("Waypoint: " + i);
        }

        //Velocity GUI
        guiPoint = HandleUtility.WorldToGUIPoint(mc.transform.position);                     
        rect = new Rect(guiPoint.x - 50f, guiPoint.y - 40, 150, 20);
        GUI.Box(rect, "Velocity: " + mc.velocity);
